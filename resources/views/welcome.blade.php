...
{{-- Tambahkan wrapper x-data untuk mengontrol mobile menu --}}
<div x-data="{ mobileMenuOpen: false }" class="relative min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 overflow-x-hidden">
    ...
    <div class="relative z-10">
        ...
        <header class="fixed top-0 left-0 right-0 z-50 px-4 pt-4 md:pt-5 transition-all duration-300">
            <div class="max-w-6xl mx-auto bg-white/80 dark:bg-gray-900/80 backdrop-blur-md rounded-2xl md:rounded-full shadow-xl px-4 md:px-6 py-3 border border-white/30 dark:border-gray-700">
                ...
                <div x-show="mobileMenuOpen" x-transition class="md:hidden mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-col space-y-3 pb-2">
                    <a href="/" class="text-green-600 font-bold">Home</a>
                    <a href="/datasets" class="text-gray-700 dark:text-gray-300 font-medium">Datasets</a>
                    <a href="/organizations" class="text-gray-700 dark:text-gray-300 font-medium">Organizations</a>
                    <a href="/groups" class="text-gray-700 dark:text-gray-300 font-medium">Groups</a>
                    <a href="/about" class="text-gray-700 dark:text-gray-300 font-medium">About</a>
        ...
        {{-- HERO SECTION --}}
        <section class="pt-40 md:pt-52 flex flex-col items-center justify-center px-4 pb-16 md:pb-20">
            <div class="max-w-4xl w-full text-center transition-all duration-300">
                <h2 class="text-3xl md:text-5xl font-extrabold mb-4 leading-tight text-white drop-shadow-md">Selamat Datang di Portal Satu Data<br class="hidden md:block"> Kabupaten Sidoarjo</h2>
                <p class="text-sm md:text-lg mb-8 md:mb-10 max-w-2xl mx-auto text-gray-100 font-medium drop-shadow">Sumber data resmi untuk pembangunan daerah yang lebih berkualitas</p>
                
                <form x-data="{ target: '/datasets' }" :action="target" method="GET" class="relative max-w-2xl mx-auto flex flex-col md:flex-row items-center bg-white/90 dark:bg-gray-800/90 border border-white/50 dark:border-gray-600 rounded-2xl md:rounded-full shadow-2xl backdrop-blur-sm p-2 md:p-0 transition-all focus-within:ring-2 focus-within:ring-green-500">
                    <div class="relative flex items-center w-full md:w-auto border-b md:border-b-0 border-gray-200 dark:border-gray-700">
                        <select x-model="target" class="w-full md:w-auto appearance-none bg-transparent px-4 py-3 md:py-4 text-xs md:text-[11px] font-black text-green-700 dark:text-green-500 border-none outline-none focus:ring-0 cursor-pointer rounded-t-2xl md:rounded-l-full md:rounded-t-none uppercase tracking-wider text-center md:text-left">
                            <option value="/datasets">Datasets</option>
                            <option value="/organizations">Organizations</option>
                            <option value="/groups">Groups</option>
                        </select>
                    </div>
                    <div class="hidden md:block h-6 w-[2px] bg-gray-200 dark:bg-gray-700"></div>
                    <div class="relative w-full flex items-center">
                        <input type="text" name="q" placeholder="Ketik kata kunci..." class="w-full pl-4 pr-12 py-3 md:py-4 text-sm font-bold bg-transparent border-none outline-none focus:ring-0 placeholder-gray-400 text-gray-800 dark:text-gray-100 rounded-b-2xl md:rounded-r-full">
                        <button type="submit" class="absolute right-4 text-xl opacity-60 hover:opacity-100 hover:scale-110 hover:text-green-600 transition-all focus:outline-none">🔍</button>
                    </div>
                </form>
            </div>
        </section>
        ...
        <div class="max-w-6xl mx-auto w-full px-4 md:px-6" x-data="{ activeCard: null }">
            <div class="flex flex-col lg:flex-row gap-4 h-auto lg:h-32 transition-all duration-500 ease-in-out">
                ...
                <div @mouseenter="activeCard = 1" @mouseleave="activeCard = null" @click="activeCard = activeCard === 1 ? null : 1"
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="flex flex-col">
                            <span class="text-[10px] font-black uppercase tracking-widest transition-colors duration-500" :class="activeCard === 1 ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'">Datasets</span>
                            <h3 class="text-xl md:text-2xl font-black transition-all duration-500" :class="activeCard === 1 ? 'text-white scale-105' : 'text-gray-800 dark:text-white'">
                                {{ $totalAll }}
                            </h3>
                        </div>
                    </div>
                ...
                <div @mouseenter="activeCard = 2" @mouseleave="activeCard = null" @click="activeCard = activeCard === 2 ? null : 2"
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="flex flex-col">
                            <span class="text-[10px] font-black uppercase tracking-widest transition-colors duration-500" :class="activeCard === 2 ? 'text-purple-100' : 'text-gray-500 dark:text-gray-400'">Organizations</span>
                            <h3 class="text-xl md:text-2xl font-black transition-all duration-500" :class="activeCard === 2 ? 'text-white scale-105' : 'text-gray-800 dark:text-white'">
                                {{ $opdTerdaftar }}
                            </h3>
                        </div>
                    </div>
                ...
                <div @mouseenter="activeCard = 3" @mouseleave="activeCard = null" @click="activeCard = activeCard === 3 ? null : 3"
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="flex flex-col">
                            <span class="text-[10px] font-black uppercase tracking-widest transition-colors duration-500" :class="activeCard === 3 ? 'text-orange-100' : 'text-gray-500 dark:text-gray-400'">Unduhan</span>
                            <h3 class="text-xl md:text-2xl font-black transition-all duration-500" :class="activeCard === 3 ? 'text-white scale-105' : 'text-gray-800 dark:text-white'">
                                {{ $totalDownload }}
                            </h3>
                        </div>
                    </div>
                ...
                <div @mouseenter="activeCard = 4" @mouseleave="activeCard = null" @click="activeCard = activeCard === 4 ? null : 4"
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="flex flex-col">
                            <span class="text-[10px] font-black uppercase tracking-widest transition-colors duration-500" :class="activeCard === 4 ? 'text-green-100' : 'text-gray-500 dark:text-gray-400'">Metadata</span>
                            <h3 class="text-xl md:text-2xl font-black transition-all duration-500" :class="activeCard === 4 ? 'text-white scale-105' : 'text-gray-800 dark:text-white'">
        ...
        {{-- AREA PETA SEBARAN --}}
        <section class="py-16 md:py-24 bg-white dark:bg-gray-950 border-t border-gray-100 dark:border-gray-900 mt-8 md:mt-0">
            <div class="max-w-6xl mx-auto px-4 md:px-6">
                <div class="flex items-center space-x-2 mb-6 md:mb-10 text-left">
                    <span class="text-2xl">🗺️</span>
                    <h3 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white border-b-4 border-green-500 pb-1">Sebaran Data Per Kecamatan</h3>
                </div>
                
                <div class="bg-gray-50 dark:bg-gray-900/60 rounded-2xl md:rounded-[32px] p-2 md:p-4 border border-gray-100 dark:border-gray-800 shadow-lg relative overflow-hidden">
                    <div id="map" class="w-full h-[350px] md:h-[500px] rounded-xl md:rounded-[24px] z-0"></div>
                </div>
            </div>
        </section>
        ...
        <section class="py-16 md:py-20 bg-white dark:bg-gray-950">
            <div class="max-w-6xl mx-auto px-4 md:px-6">
                <div class="flex items-center space-x-2 mb-6 text-left">
                    ...
                    <h3 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white border-b-4 border-green-500 pb-1">Analitik Data Sektoral</h3>
                </div>
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-gray-50 dark:bg-gray-900 rounded-2xl md:rounded-[28px] p-5 md:p-6 border border-gray-100 dark:border-gray-800 shadow-lg relative overflow-hidden group text-left">
                        <h4 class="font-black text-xs uppercase tracking-wider text-gray-500 dark:text-white/35 mb-2">Produsen Data Teratas</h4>
                        <div class="flex items-center mb-6">
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded border uppercase" style="background:rgba(52,211,153,0.12);color:#10b981;border-color:rgba(52,211,153,0.25)">Insight</span>
                            <p class="text-xs md:text-sm text-gray-600 dark:text-white/40 ml-2">OPD aktif: <span class="font-bold text-green-600 dark:text-white/75">{{ $insightOpd ?? '0' }}</span></p>
                        </div>
                        <div class="h-48 md:h-64 w-full"><canvas id="opdChart"></canvas></div>
                        <div id="opdLegend" class="flex flex-wrap gap-2.5 mt-3"></div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-900 rounded-2xl md:rounded-[28px] p-5 md:p-6 border border-gray-100 dark:border-gray-800 shadow-lg relative overflow-hidden flex flex-col group text-left">
                        <h4 class="font-black text-xs uppercase tracking-wider text-gray-500 dark:text-white/35 mb-2">Distribusi Satuan</h4>
                        <div class="flex items-center mb-4">
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded border uppercase" style="background:rgba(99,179,237,0.15);color:#3b82f6;border-color:rgba(99,179,237,0.2)">Insight</span>
                            <p class="text-xs md:text-sm text-gray-600 dark:text-white/40 ml-2">Mayoritas: <span class="font-bold text-blue-600 dark:text-white/75">{{ $insightUnit ?? '-' }}</span></p>
                        </div>
                        <div class="flex-1 min-h-[200px] flex items-center justify-center"><canvas id="unitChart"></canvas></div>
                    </div>
                    <div class="lg:col-span-3 bg-gray-50 dark:bg-gray-900 rounded-2xl md:rounded-[28px] p-5 md:p-6 border border-gray-100 dark:border-gray-800 shadow-lg relative overflow-hidden group text-left">
                        <h4 class="font-black text-xs uppercase tracking-wider text-gray-500 dark:text-white/35 mb-2">Tren Pertumbuhan Dataset</h4>
                        <div class="h-48 md:h-56 w-full"><canvas id="trendChart"></canvas></div>
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="max-w-6xl mx-auto px-4 md:px-6">
                <div class="flex flex-col lg:flex-row gap-6 items-stretch">
                    ...
                    <div class="w-full lg:w-3/4 text-left" x-data="{ 
                        ...
                        slideLeft() { this.scrollContainer.scrollBy({ left: -280, behavior: 'smooth' }); this.resetAutoScroll(); },
                        slideRight() { 
                            ...
                            } else { this.scrollContainer.scrollBy({ left: 280, behavior: 'smooth' }); }
                        ...
                        <div class="flex justify-between items-center mb-6 md:mb-8">
                            <h3 class="text-lg md:text-xl font-bold flex items-center"><span class="mr-2">📁</span> Dataset Terbaru</h3>
                        ...
                        <div class="flex overflow-x-auto gap-4 md:gap-5 pb-6 snap-x snap-mandatory hide-scrollbar" x-ref="slider" @mouseenter="clearInterval(autoScrollInterval)" @mouseleave="startAutoScroll()" @touchstart="clearInterval(autoScrollInterval)" @touchend="startAutoScroll()" style="scrollbar-width: none; -ms-overflow-style: none;">
                            @php 
                            ...
                            <div class="group relative overflow-hidden rounded-2xl md:rounded-[24px] p-5 md:p-6 transition-all hover:shadow-xl hover:-translate-y-1 border border-white dark:border-gray-800 bg-gradient-to-br {{ $style['bg'] }} dark:from-gray-800/90 dark:to-gray-900/90 flex-none w-[260px] md:w-[300px] flex flex-col snap-center cursor-pointer text-left" onclick="window.location.href='/datasets/{{ $dataset->id }}'">
                                <div class="flex justify-between items-start mb-4 md:mb-5">
                                    <span class="px-3 py-1 bg-white/80 dark:bg-gray-700/80 text-[10px] font-bold uppercase tracking-widest {{ $style['text'] }} rounded-full shadow-sm">XLSX</span>
                                    <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $dataset->created_at->diffForHumans() }}</span>
                                </div>
                                <h4 class="text-[13px] md:text-[14px] font-extrabold text-gray-800 dark:text-white mb-3 leading-snug uppercase line-clamp-3 flex-1">{{ $dataset->name }}</h4>
                                <div class="pt-4 border-t border-gray-200 dark:border-gray-700/50 flex justify-between items-center">
                                    <p class="text-[11px] font-bold text-gray-500 uppercase tracking-tighter">{{ Str::limit($dataset->user->name ?? 'Dinas Terkait', 18) }}</p>
                    ...
                    <div class="w-full lg:w-1/4">
                        <div class="bg-white/80 dark:bg-gray-900/40 backdrop-blur-xl rounded-2xl md:rounded-[28px] p-5 border border-white/60 dark:border-gray-800 shadow-lg h-full flex flex-col text-left">
                            <div class="flex items-center space-x-3 mb-5">
                                <div class="p-2 bg-orange-100 dark:bg-orange-900/30 text-orange-600 rounded-xl text-lg">🏆</div>
                                <div>
                                    <h3 class="font-bold text-sm md:text-base leading-none uppercase tracking-tight">Top Organisasi</h3>
                                    <p class="text-[10px] text-gray-500 mt-1 uppercase font-bold tracking-widest leading-none">Kontribusi</p>
                                </div>
                            </div>
                            <div class="space-y-3 flex-1">
                                @foreach($top_organisasi ?? [] as $index => $org)
                                <div class="flex items-center p-3 rounded-[20px] transition-all hover:bg-white/90 dark:hover:bg-gray-800/80 group cursor-pointer" onclick="window.location.href='/organizations/{{ $org['id'] }}'">
                                    <div class="w-10 h-10 min-w-[40px] rounded-full flex items-center justify-center font-bold text-sm {{ $index == 0 ? 'bg-yellow-400 text-white' : ($index == 1 ? 'bg-gray-300' : 'bg-orange-400 text-white') }}">{{ $index + 1 }}</div>
                                    <div class="ml-4 flex-1">
                                        <h4 class="text-xs md:text-sm font-bold text-gray-800 dark:text-gray-100 uppercase leading-tight line-clamp-2">{{ $org['name'] }}</h4>
                                        <p class="text-[10px] text-gray-500 font-bold uppercase mt-1">{{ $org['datasets'] }} Datasets</p>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                            <a href="/organizations" class="mt-5 block w-full py-3 bg-gray-100 dark:bg-gray-800 rounded-xl text-xs font-black text-center uppercase tracking-[0.1em] text-gray-500 hover:text-green-700 transition-all shadow-sm">Lihat Semua</a>
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
            ...
            L.geoJson(geojsonData, {
                ...
                onEachFeature: function(feature, layer) {
                    ...
                    const tooltipContent = `
                        <div class="p-1.5 min-w-[220px] md:min-w-[260px]">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-1">
                                <div class="bg-gray-50 p-2 rounded border border-gray-100 text-center overflow-hidden">
                                    <p class="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 truncate">Data Set</p>
                                    <p class="text-xs font-black text-gray-700">${count}</p>
                                </div>
                                <div class="bg-gray-50 p-2 rounded border border-gray-100 text-center overflow-hidden">
                                    <p class="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 truncate">Data Baru</p>
                                    <p class="text-xs font-black text-gray-700">${layanan}</p>
                                </div>
                                <div class="bg-gray-50 p-2 rounded border border-gray-100 text-center overflow-hidden">
                                    <p class="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 truncate">Pertumbuhan</p>
                                    <p class="text-xs font-black ${pertumbuhanColor}">${pertumbuhanStr}</p>
                                </div>
                                <div class="bg-gray-50 p-2 rounded border border-gray-100 text-center overflow-hidden">
                                    <p class="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 truncate">Satuan</p>
                                    <p class="text-[10px] font-black text-gray-700 truncate w-full" title="${satuan}">${satuan}</p>
                                </div>
                            </div>
                            <p class="text-[9px] text-gray-400 mt-2 uppercase tracking-widest animate-pulse text-center">(Klik untuk detail)</p>
                        </div>
                    `;
    ...
    if(document.getElementById('opdChart')) {
    ...
    if(document.getElementById('unitChart')) {
        new Chart(document.getElementById('unitChart'), {
            ...
            options: { responsive: true, maintainAspectRatio: false, cutout: '70%', plugins: { legend: { position: 'bottom', labels: { font: { weight: '600' }, usePointStyle: true, boxWidth: 10 } } } }
        });
    }
