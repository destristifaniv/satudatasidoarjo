...
<div class="flex min-h-screen bg-[#F8F9FA] dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors duration-500 overflow-x-hidden relative" 
    ...
    <main :class="sidebarOpen ? 'lg:ml-72' : 'ml-0'" class="flex-1 min-h-screen flex flex-col relative transition-all duration-500 overflow-y-auto">
        
        <div x-show="sidebarOpen" x-cloak class="fixed inset-0 z-40 bg-black/30 lg:hidden" @click="sidebarOpen = false"></div>
        
        <div class="fixed top-0 right-0 w-72 h-72 sm:w-[320px] sm:h-[320px] md:w-[400px] md:h-[400px] bg-teal-200/20 dark:bg-teal-900/10 blur-[100px] rounded-full -z-10 opacity-50"></div>
        
        <div class="p-4 sm:p-6 flex items-center z-20 relative h-16 sm:h-20 absolute top-0 left-0 w-full" :class="sidebarOpen ? 'lg:hidden' : 'block'">
            <button @click="sidebarOpen = true" class="p-2.5 rounded-xl bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" /></svg>
            </button>
        </div>

        <div class="flex-1 flex flex-col justify-center items-center w-full px-4 sm:px-6 lg:px-8 pb-8 z-10 relative">
            
            <div class="w-full max-w-4xl">
                
                <header class="mb-6 lg:mb-8 text-left mt-10 lg:mt-0">
                ...
                <div class="bg-white/70 dark:bg-gray-900/70 backdrop-blur-2xl rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-white dark:border-gray-800 p-5 sm:p-8 relative overflow-hidden">
                    ...
                    <form action="{{ route('admin.upload.dataset.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 relative z-10 text-left"
                        ...
                        <div class="relative">
                            <label 
                                ...
                                <div class="relative z-10 group-hover:scale-105 transition-transform duration-500">
                                    <div class="w-12 h-12 bg-white dark:bg-gray-800 rounded-xl flex items-center justify-center mb-3 shadow-lg shadow-teal-500/10 mx-auto border border-gray-100 dark:border-gray-700">
                                        <span class="text-2xl" x-show="!fileName">📁</span>
                                        <span class="text-2xl" x-show="fileName" x-cloak>✅</span>
                                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                            ...
                            <div class="space-y-1.5 group relative" @click.away="showDssd = false">
                                ...
                                <input type="text" name="dssd_code" x-model="dssdInput" @focus="showDssd = true" @input="showDssd = true" required autocomplete="off" 
                                       class="w-full px-5 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 border border-transparent focus:border-teal-500/30 focus:bg-white dark:focus:bg-gray-800 focus:ring-4 focus:ring-teal-500/10 outline-none text-[11px] font-bold transition-all shadow-sm" placeholder="">
                                
                                <div x-show="showDssd && filteredDssd.length > 0" x-transition class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-2xl max-h-48 overflow-y-auto custom-scrollbar" x-cloak>
                            ...
                            <div class="space-y-1.5 group relative" @click.away="showName = false">
                                ...
                                <div x-show="showName && filteredName.length > 0" x-transition class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-2xl max-h-48 overflow-y-auto custom-scrollbar" x-cloak>
                            ...
                            <div class="space-y-1.5 group">
                                ...
                                <input type="text" name="organization" value="{{ Auth::user()->name }}" readonly class="w-full px-5 py-3.5 rounded-2xl bg-gray-100 dark:bg-gray-800/80 border border-transparent text-[11px] font-bold outline-none shadow-sm cursor-not-allowed text-gray-500">
                            </div>
                            ...
                            <div class="space-y-1.5 group">
                                ...
                                <input type="text" name="tags" required placeholder="penduduk, 2024" value="{{ old('tags') }}" class="w-full px-5 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 border border-transparent focus:border-teal-500/30 focus:bg-white dark:focus:bg-gray-800 focus:ring-4 focus:ring-teal-500/10 outline-none text-[11px] font-bold transition-all shadow-sm">
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 pt-3 border-t border-gray-100 dark:border-gray-800/60">
                            <div class="space-y-1.5 group mt-3">
                                ...
                                <input type="text" name="unit" placeholder="Dokumen / Bangunan" value="{{ old('unit') }}" class="w-full px-5 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 border-none text-[10px] font-bold outline-none shadow-sm focus:ring-4 focus:ring-teal-500/10 transition-all">
                            </div>
                            <div class="space-y-1.5 group mt-3">
                                ...
                                <div class="relative">
                                    <select name="frequency" class="w-full px-5 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 border-none text-[10px] font-bold outline-none shadow-sm appearance-none cursor-pointer focus:ring-4 focus:ring-teal-500/10 transition-all">
                            ...
                            <div class="space-y-1.5 group mt-3">
                                ...
                                <input type="text" name="level" placeholder="Kecamatan / Desa" value="{{ old('level') }}" class="w-full px-5 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 border-none text-[10px] font-bold outline-none shadow-sm focus:ring-4 focus:ring-teal-500/10 transition-all">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                            <div class="space-y-1.5 group">
                                ...
                                <input type="number" name="year_start" required min="2000" max="2100" value="{{ date('Y') }}" class="w-full px-5 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 border border-transparent focus:border-teal-500/30 focus:bg-white dark:focus:bg-gray-800 outline-none text-[11px] font-bold shadow-sm focus:ring-4 focus:ring-teal-500/10 transition-all">
                            </div>
                            <div class="space-y-1.5 group">
                                ...
                                <input type="number" name="year_end" required min="2000" max="2100" value="{{ date('Y') }}" class="w-full px-5 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 border border-transparent focus:border-teal-500/30 focus:bg-white dark:focus:bg-gray-800 outline-none text-[11px] font-bold shadow-sm focus:ring-4 focus:ring-teal-500/10 transition-all">
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5 group pt-2">
                            ...
                            <textarea name="description" rows="2" 
                                    class="w-full px-5 py-3.5 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 border border-transparent focus:border-teal-500/30 focus:bg-white dark:focus:bg-gray-800 outline-none text-[11px] font-medium leading-relaxed transition-all shadow-sm focus:ring-4 focus:ring-teal-500/10">{{ old('description') }}</textarea>
                        </div>

                        <div class="pt-6 border-t border-gray-100 dark:border-gray-800 flex justify-end">
                            <button type="submit" class="w-full sm:w-auto px-10 py-4 sm:py-3.5 bg-teal-600 text-white rounded-2xl sm:rounded-full font-black text-[10px] md:text-[9px] uppercase tracking-[0.2em] shadow-lg shadow-teal-500/30 hover:bg-teal-700 hover:scale-[1.02] active:scale-95 transition-all text-center">
                                Ajukan Dataset
                            </button>
                        </div>
                    </form>
...
<style>
    /* Styling untuk scrollbar custom di dalam dropdown Alpine */
