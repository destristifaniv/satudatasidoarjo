            ...
            @if($errors->any())
                <div class="mb-5 p-4 bg-red-50/80 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-2xl flex items-start gap-3 shadow-sm animate-pulse">
                    ...
                    <div class="text-left flex-1">
                        <p class="text-[9px] font-black text-red-600 dark:text-red-400 uppercase tracking-widest mb-1 text-left">Gagal Masuk</p>
                        <p class="text-[10px] font-bold text-red-500/80 dark:text-red-300/80 leading-tight text-left">
                            @if($errors->has('email') || $errors->has('password'))
                                Email atau kata sandi yang Anda masukkan tidak sesuai. Silakan coba lagi.
                            @else
                                Pastikan semua kolom sudah diisi dengan benar.
                            @endif
                        </p>
                    </div>
                </div>
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-4">
                ...
                <div class="space-y-1.5">
                    <div class="flex justify-between items-center px-3">
                        <label class="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[1px] block text-left">
                            Password
                        </label>
                        <a href="{{ route('password.request') }}" class="text-[9px] font-bold text-green-600 hover:underline transition-colors text-left">
                            Lupa Password?
                        </a>
                    </div>
                            ...
                            <template x-if="!showPassword">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                    ...
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        M9.88 9.88L4.34 4.34
                                        m15.32 15.32l-5.34-5.34
                                        M17.614 6.386A10.05 10.05 0 0121.542 12
                                        c-1.274 4.057-5.064 7-9.542 7
                                        -1.17 0-2.285-.202-3.32-.572
